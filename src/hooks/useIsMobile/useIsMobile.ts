...
const DEFAULT_MOBILE_BREAKPOINTS = {
  sm: 640,
  md: 768,
  lg: 1024,
} as const;

type MobileBreakpointKind = keyof typeof DEFAULT_MOBILE_BREAKPOINTS;
...
function resolveMaxWidthPx(kind: string, breakpoints?: Record<string, number>): number {
  ...
  if (typeof px !== "number" || Number.isNaN(px)) {
    throw new Error(
      `useIsMobile: "${kind}"에 해당하는 너비(px)가 없습니다. breakpoints에 포함해 주세요.`,
    );
  }
  return px;
}
 ...
 *
 * @example
 * const isMobile = useIsMobile({ kind: "md" });
 * useIsMobile({ kind: "md", breakpoints: { md: 800 } });
 * useIsMobile({ kind: "tablet", breakpoints: { tablet: 900 } });
 */
export function useIsMobile({ kind, breakpoints }: UseIsMobileOptions): boolean {
